...
int hitungNode(BinTree root){
    ...
    if (isTreeEmpty(root)) {
        ...
        if (isUnerLeft(root)) {
            count++;
            count += hitungNode(LEFT(root));
        } else if (isUnerRight(root)) {
            count++;
            count += hitungNode(RIGHT(root));
        } else {
...
int hitungUang(BinTree root) {
    ...
    if (LEFT(root) != NULL) {
        curi += hitungUang(LEFT(LEFT(root))) + hitungUang(RIGHT(LEFT(root)));
    }
    if (RIGHT(root) != NULL) {
        curi += hitungUang(LEFT(RIGHT(root))) + hitungUang(RIGHT(RIGHT(root)));
    }

    int notcuri = hitungUang(LEFT(root)) + hitungUang(RIGHT(root));
